...
function Enfeeble( keys )
	...
	local max_attack_damage = target:GetBaseDamageMax()
	local min_attack_damage = target:GetBaseDamageMin()
	local average_attack = max_attack_damage - ((max_attack_damage - min_attack_damage) / 2)
	local amount_of_stacks_needed = average_attack / 5

	-- Remove current stacks and enfeeble debuff
	target:RemoveModifierByName(modifier_dmg)

    Timers:CreateTimer(function()
    	local current_stacks = target:GetModifierStackCount(modifier_dmg, caster)
        
        if (current_stacks < (amount_of_stacks_needed - 1)) then

      	   AddStacks(ability, caster, target, modifier_dmg, 1, true)
           return 1.0
