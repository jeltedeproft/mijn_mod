...
function findfriends( keys )
	local caster = keys.caster
	local ability = keys.ability
	local ability_level = ability:GetLevel() - 1
	local pos = caster:GetAbsOrigin()
	local modifier_grave = keys.modifier_grave_aoe

	local radius_grave = ability:GetLevelSpecialValueFor("radius", ability_level)

	local friends = FindUnitsInRadius(caster:GetTeamNumber(), pos, nul, radius_grave, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
	
	for _,friend in pairs(friends) do
		ability:ApplyDataDrivenModifier(caster, friend, modifier_grave, {})
	end
end
