...
function CullingBlade( keys )
	...
	local stun_duration_scepter = ability:GetLevelSpecialValueFor("stun_duration_scepter", ability_level)
	...
	if target:GetHealth() <= kill_threshold then
		...
		local units_to_buff = FindUnitsInRadius(caster:GetTeam(), caster_location, nil, speed_aoe, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO , 0, FIND_CLOSEST, false)
		...
		-- Find the enemy units in the map that should recieve the stun and then apply it to them
		local units_to_stun = FindUnitsInRadius(caster:GetTeam(), caster_location, nil, 10000, DOTA_UNIT_TARGET_TEAM_ENEMY, ability:GetAbilityTargetType() , 0, FIND_CLOSEST, false)
		for _,v in pairs(units_to_buff) do
			if scepter == true then
				ability:ApplyDataDrivenModifier(caster, v, modifier_stun, {duration = stun_duration_scepter})
			else
				ability:ApplyDataDrivenModifier(caster, v, modifier_sprint, {duration = stun_duration})
			end
		end		
