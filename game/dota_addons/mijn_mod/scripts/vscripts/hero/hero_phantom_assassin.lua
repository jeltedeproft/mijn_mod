function StifflingDaggerAoe( keys )
	...
	local particle_name = keys.EffectName          

	units = FindUnitsInRadius(caster:GetTeamNumber(), pos, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_MECHANICAL, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
...
function BlurStacks( keys )
	...
	local phase_modifier = keys.phase_modifier
	...
	if RandomInt(1, 100) > 50 then
		ability:ApplyDataDrivenModifier(caster, caster, phase_modifier, {})
	end
end
